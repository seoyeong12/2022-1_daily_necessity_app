...
import com.example.myapplication20200706.MyApplication.Companion.email
import com.example.myapplication20200706.databinding.ActivityAuthBinding
                ...
                .addOnCompleteListener(this){task ->
                    ...
                    if(task.isSuccessful){
                        if(MyApplication.checkAuth()){
                            ...
                            Toast.makeText(baseContext,"이메일 인증이 되지않았습니다.",Toast.LENGTH_SHORT).show()
                        }
                    }
                    else{
                        Toast.makeText(baseContext,"회원가입 실패",Toast.LENGTH_SHORT).show()
        ...
        binding.btnKakaoLogin.setOnClickListener{
            ...
            val callback: (OAuthToken?, Throwable?) -> Unit = { token, error ->
                if (error != null) {
                    ...
                    //MyApplication.auth.updateCurrentUser(token.accessToken)
                    //MyApplication.auth.signInAnonymously()
                    // 사용자 정보 요청 (기본)
